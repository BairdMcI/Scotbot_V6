...
class GamesCog(commands.Cog):
    ...
    async def updateWhatgame(self, channelName: str):
        if channelName == "deadm8":
            ...
            if gameName != channel.game.name:
                channel.game.name = streamInfo.game_name
                channel.game.id = streamInfo.id
                await channel.updateStreamInfo()
            game: Game = await self.fetchGame(gameName)
            if game is not None:
                whatgame = f"{channel.displayName} is playing the {game.steamInfo.genres} {game.steamInfo.categories} game, {game.name}, developed by {game.steamInfo.developers}. | LINK: " \
                           f"store.steampowered.com/app" \
                           f"/{game.id}"
